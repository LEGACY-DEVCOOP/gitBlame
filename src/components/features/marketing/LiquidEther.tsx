...
// --- Classes and Helpers (Moved Outside Component) ---
...
class ShaderPass {
  ...
  constructor(props: ShaderPassProps, common: CommonClass) {
    ...
    this.common = common;
    this.uniforms =
      (this.props.material?.uniforms as {
...
class Advection extends ShaderPass {
  ...
  constructor(simProps: AdvectionProps, common: CommonClass) {
    super(
      {
        material: {
...
class ExternalForce extends ShaderPass {
  ...
  updateForce(props: {
    ...
    if (this.mouseMesh) {
      const material = this.mouseMesh.material as THREE.RawShaderMaterial;
      material.uniforms.force.value.set(forceX, forceY);
      material.uniforms.center.value.set(centerX, centerY);
      material.uniforms.scale.value.set(props.cursor_size, props.cursor_size);
    }
    super.update();
  }
}
...
class Viscous extends ShaderPass {
  constructor(simProps: ViscousProps, common: CommonClass) {
    super(
      {
        material: {
          vertexShader: face_vert,
...
class Divergence extends ShaderPass {
  constructor(simProps: DivergenceProps, common: CommonClass) {
    super(
      {
        material: {
          vertexShader: face_vert,
...
class Poisson extends ShaderPass {
  constructor(simProps: PoissonProps, common: CommonClass) {
    super(
      {
        material: {
          vertexShader: face_vert,
...
class Pressure extends ShaderPass {
  constructor(simProps: PressurePassProps, common: CommonClass) {
    super(
      {
        material: {
          vertexShader: face_vert,
...
class WebGLManager {
  ...
  pause() {
    this.running = false;
    const currentRaf = this.rafRef.current;
    if (currentRaf) {
...
export default function LiquidEther({
  ...
  resolution = 0.5,
  isBounce = false,
  colors = ['#F00201', '#FF4500', '#24292F'], // Custom GitBlame colors
  style = {},
  className = '',
  autoDemo = true,
  autoSpeed = 0.5,
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      ...
      if (currentResizeRaf) cancelAnimationFrame(currentResizeRaf);
    ...
    return () => {
      ...
      const currentRo = resizeObserverRef.current;
      if (currentRo) {
        try {
      ...
      const currentIo = intersectionObserverRef.current;
      if (currentIo) {
        try {
